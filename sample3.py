# 参考資料 → https://sosotata.com/scouter/
...
cascadeFile = cv2.data.haarcascades + 'haarcascade_frontalface_alt_tree.xml'
...
def draw():
    ...
    if len(faceRect) > 0:
        for fx, fy, fw, fh in faceRect:
            ...
            textColor = (0, 244, 243)
            fontSize = 64
            font = ImageFont.truetype('NikkyouSans.ttf', fontSize)
...
#             # 三角形描画
#             points = [(fx + (fw / 2), fy + (fh * 1.0)),
#                       (fx + (fw / 2) - 40, fy + (fh * 1.0) + 60),
#                       (fx + (fw / 2) + 40, fy + (fh * 1.0) + 60)]
#             cv2.drawContours(curFrame, [np.array(points)], 0, textColor, -1)
